...
struct {
  ...
  { (TCHAR*)_T("br"),     tBR,   0, 1},
  { (TCHAR*)_T("em"),     tI,    0, 0},
  { (TCHAR*)_T("font"),   tFONT, 1, 0},
  { (TCHAR*)_T("i"),      tI,    0, 0},
  { (TCHAR*)_T("p"),      tP,    0, 1},
  { (TCHAR*)_T("strong"), tB,    0, 0},
  { (TCHAR*)_T("sub"),    tSUB,  0, 0},
  { (TCHAR*)_T("sup"),    tSUP,  0, 0},
  { (TCHAR*)_T("u"),      tU,    0, 0},
};
					   ...
					   )
{
	...
	SavedDC = SaveDC(hdc);
	hfontBase = (HFONT)SelectObject(hdc, (HGDIOBJ)GetStockObject(SYSTEM_FONT));
