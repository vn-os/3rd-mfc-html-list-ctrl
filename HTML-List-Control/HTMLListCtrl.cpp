...
// @refer to https://stackoverflow.com/questions/4598872/creating-hbitmap-from-memory-buffer

HBITMAP bitmap_from_data(unsigned char* data, unsigned int count)
{
  if (data == nullptr || count == 0)
  {
    throw "missing optionmarks.bmp";
  }

  auto const stream{ ::SHCreateMemStream(data, count) };
  if (!stream)
  {
    throw "E_OUTOFMEMORY";
  }
...
CHTMLListCtrl::CHTMLListCtrl()
{
	...
	CBitmap bm;
	auto hbmp = bitmap_from_data(optionmarks, sizeof(optionmarks));
	assert(hbmp != nullptr);
	bm.FromHandle(hbmp);
	DeleteObject(hbmp);
...
int CHTMLListCtrl::CalculateItemHeight(CString sText, int nStyle, UINT uiImage,int nItemWidth)
{
	...
	else if(nStyle == HTML_TEXT)
	{
		...
		int nHeight = DrawHTML(memDC.GetSafeHdc(),sText.GetBuffer(0),sText.GetLength(),&rc,DT_LEFT|DT_CALCRECT|DT_WORDBREAK|DT_EXTERNALLEADING);
